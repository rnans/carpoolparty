...
<script>
	...
	function findList() {
		...
		var params = dparams + '&' + tparams + '&' + lparams + '&' + lparams2
				+ '&' + gparams + '&' + sparams + '&slat=' + startY + '&slng='
				+ startX + '&elat=' + endY + '&elng=' + endX;
...
<body>
					...
					<div id="categorySearch" class="col-md-12">
						<div class="row">
							...
							<div class="col-md-9 col-xd-9">
								<button type="button" name="pooltype" value="타세요" id="driver"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="driveron(this)">타세요</button>
								<button type="button" name="pooltype" value="탈래요" id="member"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="memberon(this)">탈래요</button>
							</div>
						</div>
						<div class="row">
							<div class="col-md-3 col-xd-3">카풀 기간</div>
							<div class="col-md-9 col-xd-9">
								<button type="button" id="short" name="termtype" value="단기"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="shorton(this)">단기</button>
								<button type="button" id="long" name="termtype" value="정기"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="longon(this)">정기</button>
							</div>
						</div>
						<div class="row">
							<div class="col-md-3 col-xd-3">지역</div>
							<div class="col-md-9 col-xd-9">
								<button type="button" id="se" value="서울"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="seon(this)">서울</button>
								<button type="button" id="kk" value="경기"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="kkon(this)">경기</button>
								<button type="button" id="ic" value="인천"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="icon(this)">인천</button>
								<button type="button" id="ps" value="부산"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="pson(this)">부산</button>
							</div>
						</div>
						<div class="row">
							<div class="col-md-3 col-xd-3">성별</div>
							<div class="col-md-9 col-xd-9">
								<button type="button" id="female" value="남성만"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="femaleon(this)">여성만</button>
								<button type="button" id="male" value="여성만"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="maleon(this)">남성만</button>
							</div>
						</div>
						<div class="row">
							<div class="col-md-3 col-xd-3">흡연여부</div>
							<div class="col-md-9 col-xd-9">
								<button type="button" id="smoke" value="흡연"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="smokeon(this)">흡연 가능</button>
								<button type="button" id="nonsmoke" value="비흡연"
									class="button--cate button--moema button--text-thick button--text-upper button--size-s"
									onclick="nonsmokeon(this)">흡연 불가</button>
							</div>
						</div>
					</div>
					<div class="row">
						...
						<div class="col-md-6">
							<div class="col-md-6">
								<button type="button" onclick="showCate();"
									class="button button--ujarak button--border-thin button--text-thick">상세검색</button>
							</div>
							<div class="col-md-6">
								<button type="button" onclick="findList()"
									class="button button--ujarak button--border-thin button--text-thick">검색</button>
							</div>
						</div>
					</div>
					<br>
	...
	<script>
		...
		function showImg(item, index) {
			var pDiv = document.getElementsByName('profile');

			for (var i = 0; i < pDiv.length; i++) {
				if (document.getElementById('h' + i).value == item.id) {
					pDiv.setAttribute('src', '/final02/img/' + item.filename);
				}
			}
		}
	</script>
	...
	<script>
		...
		function viewList(item, index) {
			...
			// 마커 이미지를 생성합니다
			var sMarkerImg = new daum.maps.MarkerImage(sImgSrc, imgSize,
					imgOption);
			var eMarkerImg = new daum.maps.MarkerImage(aImgSrc, imgSize,
					imgOption);
			...
			var scontent = '<div class="wrap" id="scon'+index+'">'
					+ '<input type="hidden" id="h'+index+'" value="'+item.userid+'">'
					...
					+ '        <div class="body">'
					+ '            <div class="img">'
					+ '                <img name="profile" id="simg'+index+'" class="profile" src="http://s.cmstatic.net/webclient/dres/20160419171121/images/template/profile_60x60.gif" width="73" height="70">'
					+ '           </div>'
					+ '            <div class="desc">'
					+ '                <div class="ellipsis">'
					+ item.startspot
					...
					+ item.endspot
					+ '</div>'
					+ '                <div><a href="poolEachContent.do?idx='
					+ item.idx
					+ '">상세보기</a></div>'
					+ '            </div>'
					+ '        </div>' + '    </div>' + '</div>';
			var econtent = '<div class="wrap" id="econ'+index+'">'
					...
					+ '        <div class="body">'
					+ '            <div class="img">'
					+ '                <img name="profile" id="eimg'+index+'" class="profile" src="http://s.cmstatic.net/webclient/dres/20160419171121/images/template/profile_60x60.gif" width="73" height="70">'
					+ '           </div>'
					+ '            <div class="desc">'
					+ '                <div class="ellipsis">'
					+ item.startspot
												...
												function() {
													...
													var callback = function(
														...
														if (status === daum.maps.services.Status.OK) {
															...
															if (title == '출발지') {
																document
																		.getElementById('ss').value = result[0].jibunAddress.name;
															} else if (title == '도착지') {
																document
																		.getElementById('es').value = result[0].jibunAddress.name;
															}
